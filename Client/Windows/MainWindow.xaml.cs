...
namespace Client.Windows
{
    ...
    public partial class MainWindow : BaseWindowForWPF
    {
        ...
        public static readonly string cftsDirectoryName = "CFTS";
        public static readonly string cftsFileExtensions = ".cfts";
        ...
        private IWindowEnqueuer? _offeringFileSettingWindow;
        ...
        private void LoadLocalOfferingFiles()
        {
            ...
            string filesDirectory = Path.Combine(MyConfigManager.GetConfigStringValue("DownloadingDirectory"), cftsDirectoryName);
            if (Directory.Exists(filesDirectory))
            {
                string[] files = Directory.GetFiles(filesDirectory);

                // Filter the list to include only files with the desired extension
                var filteredFiles = files.Where(f => Path.GetExtension(f).Equals(cftsFileExtensions)).ToList();
        ...
        private void TryDeleteLocalOfferingFile(OfferingFileDto offeringFileDto)
        {
            ...
            string filesDirectory = Path.Combine(MyConfigManager.GetConfigStringValue("DownloadingDirectory"), cftsDirectoryName);
            string fileName = offeringFileDto.OfferingFileIdentificator + cftsFileExtensions;
        ...
        private void btnSaveFileIdentificator_Click(object sender, RoutedEventArgs e)
        {
            if (sender is Button button && button.Tag is OfferingFileDto offeringFileDto)
            {
                ...
                // Save him for later
                string fileName = offeringFileDto.OfferingFileIdentificator + cftsFileExtensions;
                string fileDirectory = Path.Combine(MyConfigManager.GetConfigStringValue("DownloadingDirectory"), cftsDirectoryName);
        ...
        private void btnOfferingFileWindow_Click(object sender, RoutedEventArgs e)
        {
            if (sender is Button button)
            {
                Log.WriteLog(LogLevel.DEBUG, button.Name);

                if (_offeringFileSettingWindow == null || !_offeringFileSettingWindow.IsOpen())
                {
                    _offeringFileSettingWindow = BaseWindowForWPF.CreateWindow<OfferingFileSettingsWindow>();
                }
                else
                {
                    _offeringFileSettingWindow.BaseMsgEnque(new WindowStateSetMessage());
                }
            }
        }

        private void btnModifyOfferingFile_Click(object sender, RoutedEventArgs e)
        {
            if (sender is Button button && button.Tag is OfferingFileDto offeringFileDto)
            {
                Log.WriteLog(LogLevel.DEBUG, button.Name);
                ...
                if (_offeringFileSettingWindow == null || !_offeringFileSettingWindow.IsOpen())
                {
                    _offeringFileSettingWindow = BaseWindowForWPF.CreateWindow<OfferingFileSettingsWindow>(() => new OfferingFileSettingsWindow(offeringFileDto));
                }
                else
                {
                    _offeringFileSettingWindow.BaseMsgEnque(new WindowStateSetMessage());
                    _offeringFileSettingWindow.BaseMsgEnque(new StartNewDownloadMessage(offeringFileDto));
                }
            }
        }
        ...
        private async void btnDecryptFile_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (result == true)
            {
                await DataEncryptor.DecryptFile(fileDialog.FileName, "result", Certificats.GetCertificate("nodeXY", Certificats.CertificateType.Node));
            }
        }
